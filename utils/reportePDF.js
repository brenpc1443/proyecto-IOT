...
class ReportePDF {
  ...
  async crearReporte(datos, nombreArchivo = 'reporte.pdf') {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Opción 1: Si viene explícito en los datos
        if (datos.tipo === "historial") {
          esHistorial = true;
        } else if (datos.tipo === "reporte") {
          esHistorial = false;
        } else if (datos.detalles && datos.detalles.length > 0) {
          // Opción 2: Detectar por estructura
          esHistorial = datos.detalles[0].id_sesion && datos.detalles[0].numero;
        }

        console.log("Tipo de reporte detectado:", esHistorial ? "HISTORIAL" : "GENERAL");
  ...
  resumenHistorial(resumen) {
    ...
    const items = [
      { label: 'Total Sesiones', valor: resumen.total_sesiones, color: '#3b82f6', icon: '📊' },
      { label: 'Sesiones Pagadas', valor: resumen.pagadas, color: '#22c55e', icon: '✓' },
      { label: 'Sesiones Pendientes', valor: resumen.pendientes, color: '#f59e0b', icon: '⏳' },
      { label: 'Sesiones Activas', valor: resumen.activas, color: '#ef4444', icon: '▶' }
    ];
