...
class ReportePDF {
  ...
  async crearReporte(datos, nombreArchivo = 'reporte.pdf') {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Determinar tipo de reporte - usar la propiedad explícita
        let esHistorial = datos.esHistorial;
        
        if (esHistorial === undefined) {
          // Fallback: detectar por título
          esHistorial = datos.titulo.toLowerCase().includes('historial');
        }

        console.log("📄 Generando PDF:", datos.titulo);
        console.log("📝 Tipo detectado: es Historial =", esHistorial);
        console.log("📊 Detalles:", datos.detalles ? datos.detalles.length + " registros" : "0 registros");
  ...
  generarReporteHistorial(datos) {
    ...
    // Resumen ejecutivo - SIEMPRE mostrar, aunque sea 0
    this.doc
      .fontSize(13)
      .font('Helvetica-Bold')
      .fillColor('#1f2937')
    ...
    const items = [
      { label: 'Total Sesiones', valor: datos.resumen.total_sesiones || 0, color: '#3b82f6', icon: '📊' },
      { label: 'Sesiones Pagadas', valor: datos.resumen.pagadas || 0, color: '#22c55e', icon: '✓' },
      { label: 'Sesiones Pendientes', valor: datos.resumen.pendientes || 0, color: '#f59e0b', icon: '⏳' },
      { label: 'Sesiones Activas', valor: datos.resumen.activas || 0, color: '#ef4444', icon: '▶' }
    ];
    ...
    items.forEach((item, idx) => {
      ...
      this.doc.rect(posX, y, anchoCaja, 65)
      ...
      this.doc
        ...
        .text(item.valor.toString(), posX, y + 10, { width: anchoCaja, align: 'center' });

      this.doc
        .fontSize(9)
        .font('Helvetica')
        .fillColor('#4b5563')
        .text(item.label, posX + 5, y + 42, { width: anchoCaja - 10, align: 'center' });
    ...
    if (datos.detalles && datos.detalles.length > 0) {
      ...
      this.doc
        ...
        .font('Helvetica')
        .fillColor('#999')
        .text('ℹ️ Sin datos disponibles para este período', 40);
    }
  ...
  cajaInfo(fechas) {
    ...
    this.doc.text(`Período reportado: ${fechas.inicio} hasta ${fechas.fin}`, 50, y + 10);
...
resumenGeneral(resumen) {
    ...
    items.forEach((item, idx) => {
      ...
      this.doc.rect(posX, y, anchoCaja, 55)
        .fillAndStroke(item.color + '15', item.color)
        .lineWidth(1.5);
      ...
      this.doc
        ...
        .font('Helvetica-Bold')
        .fillColor(item.color)
        .text(item.valor.toString(), posX, y + 8, { width: anchoCaja, align: 'center' });

      this.doc
        .fontSize(9)
        .font('Helvetica')
        .fillColor('#4b5563')
        .text(item.label, posX + 5, y + 32, { width: anchoCaja - 10, align: 'center' });
  ...
  dibujarTablaProfesional(columnas, datos) {
    ...
    for (let i = 0; i < Math.min(datos.length, 30); i++) {
      ...
      columnas.forEach(col => {
        const anchoAjustado = col.ancho * escala;
        this.doc.rect(posX, posY, anchoAjustado, altoFila).fill(colorFondo).stroke();
  ...
  formatearValor(fila, key) {
    ...
    // Estado de pago
    if (key === 'pago') {
      return fila.pago === 1 ? '✓ Pagada' : (fila.hora_salida ? 'Pendiente' : 'Activa');
    }
  ...
  piePaginaConNumero() {
    ...
    this.doc
      ...
      .font('Helvetica')
      .fillColor('#999')
      .text('Sistema de Estacionamiento Inteligente | Reporte Confidencial', 40, y, { align: 'center' });
